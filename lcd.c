...
void lcdCommand(unsigned char cmnd)
{
	...
	LCD_PRT = (LCD_PRT & 0xF0) | (cmnd >> 4);		//Send high nibble d4-d7 to data port
	...
	LCD_PRT = (LCD_PRT & 0xF0) | (cmnd & 0x0F);;		//Send low nibble d4-d7 to data port
	LCD_PRT |= (1<<LCD_EN);			//EN=1 for H=to-L pulse
	_delay_us(1);					//Wait to make enable wide
	LCD_PRT &= ~(1<<LCD_EN);		//EN = 0 for H-to-L pulse
}
...
void lcdData(unsigned char data)
{
	...
	LCD_PRT = (LCD_PRT & 0xF0) | (data >> 4);		//Send high nibble d4-d7 to data port
	...
	LCD_PRT = (LCD_PRT & 0xF0) | (data & 0x0F);;		//Send low nibble d4-d7 to data port
	LCD_PRT |= (1<<LCD_EN);			//EN=1 for H=to-L pulse
	_delay_us(1);					//Wait to make enable wide
	LCD_PRT &= ~(1<<LCD_EN);		//EN = 0 for H-to-L pulse
